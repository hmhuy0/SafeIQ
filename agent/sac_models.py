...
class DiagGaussianActor(nn.Module):
    ...
    def get_norm_logp(self,obs,action):
        dist = self.forward(obs)
        action_clip = torch.clip(action, -1. + EPS, 1. - EPS)
        log_prob = dist.log_prob(action_clip)
        log_prob_clip = torch.clip(log_prob, LOG_PI_NORM_MIN, LOG_PI_NORM_MAX)
        log_prob_clip = (log_prob_clip - LOG_PI_NORM_MIN) / (LOG_PI_NORM_MAX - LOG_PI_NORM_MIN)
        return log_prob_clip
